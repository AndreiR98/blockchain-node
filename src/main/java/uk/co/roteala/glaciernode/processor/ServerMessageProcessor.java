...
import uk.co.roteala.glaciernode.processor.server.ServerBlockHeaderProcessor;
import uk.co.roteala.glaciernode.storage.StorageServices;

import java.time.Duration;

/**
 * Process messages incoming from clients
 * Can broadcast to:
 * Broker, Other servers
 * */
@Slf4j
@Component
public class ServerMessageProcessor implements Processor {
    @Autowired
    ...
    private void processBlockHeader(Message message) {
        ...
        final ServerBlockHeaderProcessor blockHeaderProcessor =
                new ServerBlockHeaderProcessor(blockHeader, connection, storage, clientConnectionStorage, serverConnectionStorage,
